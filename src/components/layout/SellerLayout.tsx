  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex font-['Plus_Jakarta_Sans',sans-serif]">
      ...
      <div className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 border-r border-[#E9ECEF] bg-white">
        <SellerSidebar onClose={() => setIsMobileSidebarOpen(false)} />
      </div>
      ...
      <div className="flex flex-col flex-1 md:pl-64">
        <SellerHeader onMenuToggle={() => setIsMobileSidebarOpen(!isMobileSidebarOpen)} />
        
        {/* Scrollable Context View Canvas */}
        <main className="flex-1 overflow-y-auto p-3 md:p-8 max-w-350 w-full mx-auto">
          <Outlet />
        </main>
      </div>
    </div>
